...
Token tokenizer(EasyScript_void) {
	...
	EasyScript_char c;			/* input symbol */
	EasyScript_int state = 0;	/* initial state of the FSM */
	EasyScript_int lexStart;	/* start offset of a lexeme in the input char buffer (array) */
	EasyScript_int lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	EasyScript_int lexLength;	/* token length */
	EasyScript_int i;			/* counter */
	///EasyScript_char newc;		// new char

	/* Starting lexeme */
	EasyScript_string lexeme;	/* lexeme (to check the function) */
...
Token funcCMT(EasyScript_string lexeme) {
	...
	EasyScript_int i = 0, len = (EasyScript_int)strlen(lexeme);
...
Token funcID(EasyScript_string lexeme) {
	...
	EasyScript_char lastch = lexeme[length - 1];
	EasyScript_int isID = EasyScript_FALSE;
	switch (lastch) {
		case AMP_CHR:
			currentToken.code = MNID_T;
			scData.scanHistogram[currentToken.code]++;
			isID = EasyScript_TRUE;
			break;
		default:
			// Test Keyword
...
Token funcKEY(EasyScript_string lexeme) {
	Token currentToken = { 0 };
	EasyScript_int kwindex = -1, j = 0;
	EasyScript_int len = (EasyScript_int)strlen(lexeme);
