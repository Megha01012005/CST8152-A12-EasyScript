...
EasyScript_int errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern EasyScript_int line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(EasyScript_void);
 ...
 * -------------------------------------------------------------
 */
EasyScript_void printScannerError(EasyScript_string fmt, ...);
EasyScript_void displayScanner(BufferPointer ptrBuffer);
EasyScript_long getScannerFilesize(EasyScript_string fname);
