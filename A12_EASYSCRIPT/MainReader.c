 ...
 */
  ...
  * -------------------------------------------------------------
  */
EasyScript_void bErrorPrint(EasyScript_string fmt, ...);
EasyScript_void displayBuffer(BufferPointer ptr_Buffer);
EasyScript_long getFileSize(EasyScript_string fname);
EasyScript_int isNumber(const EasyScript_string ns);
EasyScript_void startReader(EasyScript_string, EasyScript_string, EasyScript_char, EasyScript_int, EasyScript_int);
...
EasyScript_int mainReader(EasyScript_int argc, EasyScript_string* argv) {
	...
	EasyScript_string program = argv[0];
	EasyScript_string input = argv[2];
	EasyScript_char mode = MODE_FIXED;
	EasyScript_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
EasyScript_void startReader(EasyScript_string program, EasyScript_string input, EasyScript_char mode, EasyScript_int size, EasyScript_int increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	EasyScript_int loadSize = 0;		/* the size of the file loaded in the buffer */
	EasyScript_char symbol;			/* symbol read from input file */
	...
	if ((loadSize != EasyScript_ERROR) && (loadSize != 0)) {
...
EasyScript_void bErrorPrint(EasyScript_string fmt, ...) {
	...
	(EasyScript_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
EasyScript_int isNumber(const EasyScript_string ns) {
	EasyScript_char c; EasyScript_int i = 0;
	if (ns == NULL) return 0;
	while ((c = ns[i++]) == 0) {
